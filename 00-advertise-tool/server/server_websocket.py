    ...
    try:
        async for message in websocket:
            ...
            try:
                        ...
                        if request.method == "initialize":
                            ...
                            response = JSONRPCResponse(
                                ...
                                result={
                                    ...
                                    "serverInfo": {
                                        "name": "Simple MCP WebSocket Server",
                                        "version": "0.1.0"
                                    },
                                    "capabilities": {
                                        "tools": {
                            ...
                            # Respond with our tool advertisement
                            tools_result = ListToolsResult(tools=[time_tool])
                            response = JSONRPCResponse(
                                jsonrpc="2.0",
                                id=request.id,
                            ...
                            response = JSONRPCResponse(
                                ...
                                result={
                                    "content": [
                                        {
                                            "type": "text",
                                            "text": "Tool called but not implemented in this example"
                                        }
                                    ]
                                }
                            )
                ...
                # Handle chat messages (from the example)
                elif "type" in data and data["type"] == "chat.message":
                    message_text = data.get("content", {}).get("text", "")
                    logger.info(f"Chat message received: {message_text}")
                    logger.debug(f"Chat message data: {json.dumps(data, indent=2)}")
                    
                    # Send a response chat message
                    response = {
                        "type": "chat.message",
                        "content": {"text": "hi there"}
                    }
                    logger.debug(f"Sending chat response: {json.dumps(response, indent=2)}")
...
async def main():
    ...
    logger.debug(f"Tool details: {json.dumps(time_tool.model_dump(), indent=2)}")
